...
class Solution
{
 ...
 public:
  ...
  bool isValidSerialization(string preorder)
  {
    ...
    list_preorder.resize(1, '-');
    auto list_it = list_preorder.begin();
    auto str_it = preorder.begin();
    char curr_char;
    for(;str_it != preorder.end(); ++str_it)
      {
	curr_char = *str_it;
	if(curr_char == ',')
	  ...
	  if(list_preorder.back() == '-')
	    list_preorder.back() = '0';
	  else
	    continue;
	
	else
	  list_preorder.back() = curr_char;
      }
